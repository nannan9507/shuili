...
export default {
  ...
  methods: {
    getData(station) {
      ...
      const resource = this.$resource('/apis/data/raw/histories{/id}');
      let id;
      if (station) {
        id = station.id
      } else {
        ...
        resource.get({
          ...
          if (res.body.success) {
            if (!$.isEmptyObject(res.body.data)) {
              ...
              this.$broadcast('getPaginationList', []);
            }
          }
        })
      }
    ...
    simulateTimeline(item) {
      ...
      for (let i in item) {
        list[i] = { time: formatDateTime(new Date(i)), data: [], station_name: JSON.parse(sessionStorage.getItem('station')).name }
        for (let j in item[i]) {
          if (!factors[j]) {
    ...
    nodeToStation(node) {
      for (let i of this.stations) {
        if (i.id === node.id) {
          ...
          document.title = this.station.name+'-'+'原始数据';
        }
      }
    },
    'paginateTmp'(tmp) {
